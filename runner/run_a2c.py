...
def train_a2c_agent(profiling_data: ProfilingData, episodes=50000, is_test=False, verbose=True, total_pipelines=1): 
    ...
    alpha_actor = [0.01, 0.05, 0.1, 0.15, 0.2 , 0.25]
    ...
    for a in alpha_actor:
        for c in alpha_critic:
            for g in gamma:
                ...
                try: 
                    os.remove("a2c_tables.pkl")  # Remove existing model file to start fresh
                    os.remove("grouping_tables.pkl")  # Remove existing grouping agent file to start fresh
                except FileNotFoundError:
                    print(f"No saved model found for alpha_actor={a}, alpha_critic={c}, gamma={g}. Starting fresh training.")
            
                grouping_RL_agent = GroupingRL(total_pipelines=total_pipelines)

                print(f"Training with alpha_actor={a}, alpha_critic={c}, gamma={g}")    
