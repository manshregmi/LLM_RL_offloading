...
class CloudEdgeSimulator:
    ...
    def get_contention_at_time(self, time_seconds):
        ...
        if self.contention_trace is None or self.contention_trace_times is None:
            raise Exception('No contention data found')
            # return 0, 0, 0
        ...
        # comment them all out and return constant 
        return int(row["i"]), int(row["j"]), int(row["k"]), float(row['bandwidth_mbps'])
    # ================= Episode Time =================
    ...
    def get_next_state_cloud_waiting_time(self,next_layer,current_action,isAllCloud=False):
        ...
        self.i, self.j, self.k, bandwidth = self.get_contention_at_time(
            self.cumulative_time_seconds + self.episode_offset
        )
        ...
        for node_idx in cloud_nodes:
            ...
            # effective_cloud_times.append(max(0.0, (cloud_proc_ms + contention_extra)))
            effective_cloud_times.append(max(0.0, (contention_extra)))
        ...
        if isAllCloud:
            ...
            if row is None:
                ...
                yolos_extra = float(row["yolos_extra"])
                bart_extra = float(row["bart_extra"])
            new_cloud_pending = max(llama_extra, yolos_extra,bart_extra)
        return float(new_cloud_pending), (float(bandwidth)/8)
 
    # ================= Next State (Simplified - No Surplus) =================
 
    def get_next_state(self, current_state, action, new_cloud_pending, bandwidth):
        """
    ...
    def compute_latency(self, current_state, current_action, cloud_pending_ms):
        ...
        if np.any(new_current_action[:, 1] == 1):  # Any cloud execution
            # print("cloudlet waiting time is :", cloud_pending_ms)
