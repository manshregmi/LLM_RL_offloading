...
from simulator.contention_generator import TraceTracker, load_trace_data_from_csv , load_contention_map_from_csv
...
def get_contention_data(contention_csv_path, n_yolos_inference, n_llama_inference, n_bart_inference):
    ...
    return {
        "Llama contention ": float(row.iloc[0]['llama32_extra']),
        "Yolos contention": float(row.iloc[0]['yolos_extra']),
        "Bart contention": float(row.iloc[0]['bart_extra'])
    }
...
class CloudEdgeSimulator:
    ...
    def __init__(self, profiling_data: ProfilingData, bandwidth_csv_path: Optional[str] = None, total_pipeline=1):
        ...
        self.j = 0
        self.k = 0
        self.contention_trace = None 
        self.contention_trace_times = None 
        ...
        try:
            self.contention_map = load_contention_map_from_csv(self.contention_csv_path)
        ...
        if bandwidth_csv_path:
            try:
                ...
                print(f" Could not load bandwidth data: {e}")
                print("Falling back to stochastic bandwidth")
    def load_contention_trace(self, csv_path):
        """Load the contention trace generated by contention_generator.py."""
        df = pd.read_csv(csv_path)

        required = ["timestamp_s", "i", "j", "k"]
        missing = [column for column in required if column not in df.columns]
        if missing:
            raise ValueError(
                f"Contention trace missing required columns: {missing}"
            )
        ...
        if df.empty:
            raise ValueError("Contention trace contains no valid samples")

        self.contention_trace = df
        self.contention_trace_times = df["timestamp_s"].to_numpy(dtype=float)

        print(
            f"[OK] Loaded {len(df)} contention samples "
            f"({self.contention_trace_times[0]:.3f}s - "
            f"{self.contention_trace_times[-1]:.3f}s)"
        )

    def get_contention_at_time(self, time_seconds):
        """Return the most recent generated (i, j, k) contention sample."""
        if self.contention_trace is None or self.contention_trace_times is None:
            return 0, 0, 0

        query_time = float(time_seconds)
        start_time = float(self.contention_trace_times[0])
        end_time = float(self.contention_trace_times[-1])
        duration = end_time - start_time

        # Loop the trace for long training runs.
        if duration > 0.0:
            query_time = ((query_time - start_time) % duration) + start_time
        else:
            query_time = start_time

        idx = int(
            np.searchsorted(
                self.contention_trace_times,
                query_time,
                side="right",
            ) - 1
        )
        idx = max(0, min(idx, len(self.contention_trace_times) - 1))

        row = self.contention_trace.iloc[idx]
        return int(row["i"]), int(row["j"]), int(row["k"])
    # ================= Episode Time =================
    ...
    def get_segment_tuple(self, layer_index):
        ...
        for start_layer, end_layer, segments, _models in self.profiling.model_boundary_layers:
